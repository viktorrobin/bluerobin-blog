...
<ExternalCite title="CQRS Pattern" url="https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs" author="Microsoft" date="2024-06-12" />
...
<ExternalCite title="FastEndpoints Documentation" url="https://fast-endpoints.com/docs/get-started" author="Dj Mudaliar" date="2024-09-01" />
...
<ImplementationNote title="Choosing between FastEndpoints handlers and MediatR">
I went back and forth on whether to dispatch commands through MediatR or keep the logic directly in the FastEndpoints handler. In practice, I found that MediatR adds real value only when you need cross-cutting pipeline behaviors like validation, logging, or transaction management that apply uniformly across many commands. For simple endpoints with minimal logic, the FastEndpoints handler alone is cleaner and avoids an unnecessary layer of indirection. My rule of thumb now is: if the endpoint has fewer than 20 lines of logic and no shared pipeline concerns, skip MediatR.
</ImplementationNote>
...
<ExternalCite title="MediatR Wiki" url="https://github.com/jbogard/MediatR/wiki" author="Jimmy Bogard" date="2024-07-15" />
...
<ImplementationNote title="N+1 queries hiding in read projections">
I discovered a sneaky performance issue when using EF Core projections on the read side. My `Select()` expression accessed a navigation property (`d.Author.FullName`) that was not included in the query plan, causing EF Core to silently issue a separate query for each document when I fetched a list. The fix was to use explicit `Include()` before the `Select()`, or better yet, to use a raw Dapper query that joined the tables in one shot. I now always check the generated SQL via `ToQueryString()` during development to catch these hidden N+1 patterns before they reach production.
</ImplementationNote>

<ExternalCite title="EF Core Performance Tips" url="https://learn.microsoft.com/en-us/ef/core/performance/" author="Microsoft" date="2024-08-05" />
...
<ExternalCite title="Implementing CQRS in .NET" url="https://www.kamilgrzybek.com/blog/posts/simple-cqrs" author="Kamil Grzybek" date="2023-10-20" />
...
<ImplementationNote title="Eventual consistency in read model synchronization">
When I first introduced a separate read model backed by Redis, I underestimated how confusing eventual consistency would be for users. A user would create a document and immediately navigate to the list page, only to not see their new entry. The synchronization event had not been processed yet. My solution was a simple "read-your-writes" trick: after a successful command, the API response includes the created resource ID, and the frontend optimistically inserts it into the local list while the read model catches up in the background. This gave users immediate feedback without sacrificing the architectural benefits of a separate read store.
</ImplementationNote>
...
- [Microsoft CQRS pattern guide](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) -- the canonical reference for when and how to apply CQRS.
- [FastEndpoints documentation](https://fast-endpoints.com/) -- the official docs covering the REPR pattern, validation, and endpoint configuration.
- [MediatR Wiki](https://github.com/jbogard/MediatR/wiki) -- Jimmy Bogard's documentation on pipeline behaviors, notifications, and request handling.
- [Kamil Grzybek's modular monolith series](https://www.kamilgrzybek.com/blog/posts/simple-cqrs) -- a thorough walkthrough of CQRS in .NET with DDD principles.
