...
category: "frontend"
difficulty: "advanced"
tags: ["blazor", "dotnet", "event-driven", "nats", "real-time"]
series:
...
<ExternalCite title="NATS.io Documentation" url="https://docs.nats.io/nats-concepts/overview" author="Synadia Communications" date="2024-09-01" />
...
<ExternalCite title="NATS JetStream" url="https://docs.nats.io/nats-concepts/jetstream" author="Synadia Communications" date="2024-09-01" />
...
<ExternalCite title="Background tasks with hosted services in ASP.NET Core" url="https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services" author="Microsoft" date="2024-11-12" />
...
<ExternalCite title="NATS Consumers" url="https://docs.nats.io/nats-concepts/jetstream/consumers" author="Synadia Communications" date="2024-09-01" />
...
<ImplementationNote>
    I initially used a durable consumer for the web listener, thinking it would be safer to not miss events. This backfired badly: after a deployment restart, the consumer would replay thousands of stale events, flooding every connected Blazor component with outdated status updates. Switching to an ephemeral consumer with `DeliverPolicy.New` was the correct choice for UI notifications. If a user refreshes the page, they get the current state from the API; they do not need historical event replay.
</ImplementationNote>
...
<ImplementationNote>
    The `EventSubscription` helper class was born out of necessity after I tried to unsubscribe lambda event handlers directly and discovered that `event -= lambda` does not work as expected in C# because each lambda creates a new delegate instance. By capturing the exact delegate reference in the `EventSubscription` at subscription time, we guarantee that the unsubscribe call removes the correct handler. Without this pattern, handlers accumulate and components receive duplicate notifications.
</ImplementationNote>
...
<ExternalCite title="NATS .NET Client" url="https://github.com/nats-io/nats.net" author="NATS Authors" date="2024-07-15" />
...
| **AppComponentBase** | Subscription management + cleanup |
...
- [NATS JetStream Documentation](https://docs.nats.io/nats-concepts/jetstream) -- Official guide to JetStream consumers, streams, and delivery policies.
- [ASP.NET Core Blazor component lifecycle](https://learn.microsoft.com/en-us/aspnet/core/blazor/components/lifecycle) -- Understanding disposal and cleanup in Blazor Server.
- [Background tasks with hosted services](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services) -- Patterns for long-running background services in ASP.NET Core.
