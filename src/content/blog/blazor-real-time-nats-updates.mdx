...
Blazor Server maintains a persistent connection to the server, making it ideal for real-time updates. Instead of adding SignalR (which Blazor already uses internally), we'll leverage our existing NATS messaging infrastructure to push document processing updates directly to the UI.
...
First, create an in-process notification service that Blazor components can subscribe to:
...
<ImplementationNote>
Register the notifier as a singleton so all components and the NATS listener share the same instance.
</ImplementationNote>

## NATS Event Listener

Create a hosted service that bridges NATS events to the notification service:
...
public sealed class NatsDocumentEventListener : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // Ephemeral consumer for real-time notifications
...
Create a base component that handles subscriptions and cleanup:
...
A complete upload component with real-time progress:
...
A document list that updates in real-time:
...
<Callout type="tip">
By subscribing to updates at the component level, each component receives only the updates it needs. The NATS listener broadcasts to all subscribers, and filtering happens in the component.
</Callout>

## Summary

We've built a complete real-time update system for Blazor Server:

1. **Notification Service** — In-process event aggregation
2. **NATS Listener** — Bridges NATS events to the notification service
3. **Base Component** — Handles subscriptions and cleanup
4. **Upload Component** — Real-time progress during document processing
5. **Document List** — Live updates when documents change

This approach leverages existing NATS infrastructure instead of adding SignalR, keeping the architecture simpler and more consistent.

Next in the series, we'll build **advanced form components** with validation and glass morphism styling.
