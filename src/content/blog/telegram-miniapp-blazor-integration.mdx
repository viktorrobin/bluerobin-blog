...
description: "A deep dive into creating a Telegram Mini App that enables passkey authentication, leveraging Blazor Server, WebAuthn, and Telegram's native theming system."
pubDate: 2026-01-25
...
When I first configured the Telegram Mini App to work with Blazor Server, the biggest surprise was how many subtle issues lurk in the WebView environment. The Telegram SDK loaded fine, but cross-origin cookies refused to stick, the viewport kept resizing unpredictably, and my Blazor SignalR connection dropped every time the user minimized the app. It took several days of trial and error -- reading Telegram's sparse documentation, testing on multiple devices, and digging through WebView-specific quirks -- before the experience felt native. This article captures everything I learned so you can skip the pain.
...
<CodeFile path="MyApp.Web/Components/Layout/TelegramMiniappLayout.razor" language="razor">
```razor
...
<ExternalCite title="Telegram Mini Apps - ThemeParams" url="https://core.telegram.org/bots/webapps#themeparams" author="Telegram" date="2024-12-20" />

<CodeFile path="MyApp.Web/Styles/input.css" language="css">
```css
...
<ImplementationNote title="Viewport Height Flicker on iOS">
I spent an entire afternoon debugging a visual glitch where the Mini App would flicker on iOS when the Telegram keyboard appeared. The root cause was using `100vh` instead of `100dvh` for the body height. On iOS, `100vh` includes the area behind the virtual keyboard, causing a layout shift every time the keyboard toggled. Switching to `100dvh` (dynamic viewport height) and adding `maximum-scale=1.0, user-scalable=no` to the viewport meta tag fixed the flicker completely. This is easy to miss because it only reproduces on physical iOS devices, not in Chrome DevTools.
</ImplementationNote>
...
<CodeFile path="MyApp.Api/Endpoints/Auth/TelegramMiniAppLoginEndpoint.cs" language="csharp">
```csharp
/// <summary>
...
<ExternalCite title="Web Authentication: An API for accessing Public Key Credentials (Level 3)" url="https://www.w3.org/TR/webauthn-3/" author="W3C" date="2024-04-08" />

<CodeFile path="MyApp.Web/Components/Pages/Telegram/SetupPasskey.razor" language="razor">
```razor
...
<PageTitle>Setup Passkey</PageTitle>
    ...
    @switch (_state)
    {
        case PageState.Loading:
            <div class="miniapp-loading">
                ...
                <p class="tg-text-hint">Connecting...</p>
            </div>
            break;
        ...
        case PageState.Success:
            <div class="miniapp-status">
                ...
                <p class="miniapp-status-message">
                    You can now use this passkey to sign in on any device.
                </p>
...
<ExternalCite title="Telegram Bot API - MenuButtonWebApp" url="https://core.telegram.org/bots/api#menubuttonwebapp" author="Telegram" date="2024-12-20" />
...
<ExternalCite title="SameSite cookies explained" url="https://web.dev/articles/samesite-cookies-explained" author="Rowan Merewood" date="2024-02-29" />
...
```csharp
HttpContext.Response.Cookies.Append("App-TelegramSession", user.Id.Value, new CookieOptions
{
    HttpOnly = true,
    Secure = true,
...
The staging environment no longer exists (prod-only collapse, ADR-034) — the Mini App is served from the production URL only.
</Callout>
...
<ImplementationNote title="initData Validation Race Condition">
I hit a subtle race condition during testing: occasionally, the `initData` validation would fail with "Data expired" even though the user had just opened the Mini App. The issue was that Blazor Server's `OnAfterRenderAsync` sometimes fired several seconds after the WebView loaded, and the 5-minute window had nearly elapsed by then on slow network connections. My fix was to call `getTelegramInitData` immediately in JavaScript on page load and cache it, rather than waiting for the Blazor circuit to establish. This reduced the effective delay from ~4 seconds to under 100ms.
</ImplementationNote>
...
Building this integration pushed my understanding of cross-origin constraints, mobile WebView behavior, and the limits of Blazor Server in non-traditional hosting contexts. The most valuable lesson was that testing exclusively in desktop browsers is insufficient -- the real bugs only appeared on physical devices inside the actual Telegram app. If I were starting over, I would set up on-device testing from day one.
...
- **Add passkey management UI** so users can view and revoke registered passkeys from within the Mini App
- **Implement conditional UI** that detects whether the user's device supports WebAuthn before showing the passkey flow
- **Explore Telegram Cloud Storage API** for persisting Mini App state across sessions without requiring server-side storage
- **Add analytics events** to track conversion rate from Mini App open to successful passkey registration
...
<ExternalCite title="Passkeys.dev" url="https://passkeys.dev/" author="Passkeys" date="2024" />

<ExternalCite title="Blazor Server Hosting Model" url="https://learn.microsoft.com/en-us/aspnet/core/blazor/hosting-models" author="Microsoft" date="2024" />

<ExternalCite title="SameSite Cookies Explained" url="https://web.dev/articles/samesite-cookies-explained" author="Web" date="2024" />
