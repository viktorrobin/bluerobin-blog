...
This topology has moved on: the staging namespaces are gone (BlueRobin collapsed to a prod-only cluster, ADR-034), SigNoz was replaced by the Grafana LGTM stack (OTEL Collector → Tempo/Loki/Mimir), and MinIO by Cloudflare R2 (ADR-006). The organize-by-concern principle below still holds.
</Callout>
