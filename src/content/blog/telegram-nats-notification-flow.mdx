...
readTime: "15 min"
ogImage: "/og/telegram-nats-notification-flow.png"
canonicalURL: "https://blog.bluerobin.io/telegram-nats-notification-flow"
toc: true
series: "telegram-integration"
seriesOrder: 3
deprecated: true
deprecatedReason: "Telegram is no longer the primary notification channel — ntfy is primary via a channel-agnostic dispatcher (ADR-035)."
---
...
When I first configured the NATS notification pipeline, the most frustrating issue was silence. I would upload a document, see it fully processed in the logs, but no Telegram notification arrived. The problem? My API was publishing to `staging.notifications.telegram` while the bot was subscribed to `notifications.telegram` -- no environment prefix. There was no error, no warning, just two services talking past each other on different subjects. That debugging session is what convinced me to build the `IEventSubjectProvider` abstraction described in this article. It also taught me that fire-and-forget messaging demands extra discipline around observability, since there is no built-in feedback loop when something goes wrong.

## Introduction

When a user uploads a document to our platform, it goes through OCR, classification, entity extraction, chunking, and embedding generation. This can take anywhere from 30 seconds to several minutes. Users shouldn't have to refresh the page to know when processing completes—they should get a notification directly in Telegram.
...
<ExternalCite title="NATS Subject-Based Messaging" url="https://docs.nats.io/nats-concepts/subjects" author="Synadia" date="2024-10-01" />
...
<CodeFile path="MyApp.Application/Services/EventSubjectProvider.cs" language="csharp">
```csharp
...
<ExternalCite title="NATS.Net Client Library" url="https://github.com/nats-io/nats.net" author="NATS Authors" date="2024-09-15" />

<CodeFile path="MyApp.Infrastructure/Notifications/TelegramNotificationService.cs" language="csharp">
```csharp
...
<ExternalCite title="Telegram Bot API - sendMessage" url="https://core.telegram.org/bots/api#sendmessage" author="Telegram" date="2024-12-20" />

<CodeFile path="MyApp.TelegramBot/Services/TelegramNotificationWorker.cs" language="csharp">
```csharp
/// <summary>
...
<ExternalCite title="NATS JetStream Documentation" url="https://docs.nats.io/nats-concepts/jetstream" author="Synadia" date="2024-10-01" />
...
<ExternalCite title="Kustomize - Kubernetes Native Configuration Management" url="https://kustomize.io/" author="Kubernetes SIG CLI" date="2024-06-01" />
...
<ImplementationNote title="Notification Deduplication Surprises">
During early testing, I noticed users occasionally receiving duplicate Telegram notifications for the same document. The root cause was that the OCR worker sometimes published the `embeddings.completed` event twice -- once from the embedding step and once from a retry path that fired when the Qdrant upsert timed out but ultimately succeeded. Since Core NATS offers at-most-once delivery per publish, two publishes meant two notifications. I solved this by adding a `MessageId` field to the notification DTO and keeping a short-lived LRU cache (60-second TTL) in the `TelegramNotificationWorker` to skip duplicates. It is a simple fix, but it took a while to reproduce the issue reliably.
</ImplementationNote>
...
Looking back, the decision to use Core NATS for notifications and JetStream for document events was the right call, but it was not obvious at the time. I initially built everything on JetStream, thinking guaranteed delivery was universally better. In practice, the JetStream consumer state added operational complexity that was disproportionate for ephemeral notifications. Simplifying to Core NATS for the notification path cut the debugging surface in half and made the system easier to reason about.
...
- **Add notification batching** to coalesce multiple rapid events (e.g., batch-upload of 20 documents) into a single summary message
- **Implement delivery receipts** by tracking Telegram API response status and exposing a "notification health" endpoint
- **Build a dead-letter subject** for notifications that fail Telegram API delivery after retries, enabling manual re-send from an admin UI
- **Add priority-based routing** so high-priority failure notifications bypass any batching or rate-limiting logic
...
<ExternalCite title="NATS.Net Client Documentation" url="https://github.com/nats-io/nats.net" author="GitHub Community" date="2024" />

<ExternalCite title="Telegram Bot API - sendMessage" url="https://core.telegram.org/bots/api#sendmessage" author="Telegram" date="2024" />

<ExternalCite title="OpenTelemetry .NET" url="https://opentelemetry.io/docs/languages/net/" author="OpenTelemetry Authors" date="2024" />

<ExternalCite title="Kustomize Documentation" url="https://kustomize.io/" author="Kubernetes SIG CLI" date="2024" />
