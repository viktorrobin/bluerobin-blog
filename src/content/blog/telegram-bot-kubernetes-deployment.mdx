...
seriesOrder: 4
deprecated: true
deprecatedReason: "Deploys the legacy Telegram notification bot with staging overlays — ntfy is now primary (ADR-035) and the cluster is prod-only (ADR-034)."
---
...
- **[GitOps with Flux CD](/articles/gitops-flux-cd-introduction)** -- deep dive into the Flux reconciliation loop that automates our deployments.
- **[External Secrets with Infisical](/articles/external-secrets-infisical-kubernetes)** -- how we manage secrets across staging and production.
- **[Container Security Best Practices](/articles/container-security-best-practices-dotnet)** -- hardening the Docker images we built in this article.
