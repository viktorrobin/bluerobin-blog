...
<ExternalCite
  ...
  author="Johnson, J., Douze, M. & Jegou, H."
  date="2019"
/>
...
<ExternalCite
  title="Qdrant Configuration and Optimization"
  url="https://qdrant.tech/documentation/guides/configuration/"
  author="Qdrant"
  date="2024"
/>

<ImplementationNote title="On-Disk Payload Memory Savings">
Setting `on_disk_payload: true` reduced our memory usage by roughly 60%. Payload data (text content, metadata JSON) was stored on disk while only the vector data and HNSW index remained in memory. For our 500K+ vector collection, this meant the difference between needing 4GB and 1.5GB of RAM per node. The trade-off is slightly higher latency when reading payload fields during search, but since we only fetch payloads for the final top-K results (not during the ANN search itself), the impact was negligible in practice -- maybe 5-10ms additional per query.
</ImplementationNote>
...
<ExternalCite
  title="Qdrant Distributed Deployment (Clustering Guide)"
  url="https://qdrant.tech/documentation/guides/distributed_deployment/"
  author="Qdrant"
  date="2024"
/>

<ImplementationNote title="Headless Service for Peer Discovery">
The Qdrant cluster peer discovery required a headless service -- and this was one of those bugs that took me far too long to figure out. I initially used a regular ClusterIP service for everything, and the Qdrant nodes simply could not find each other for Raft consensus. The logs showed "peer connection timeout" errors that I initially blamed on networking policies. The actual fix was the headless service with `clusterIP: None`, which creates DNS A records for each individual pod (e.g., `qdrant-0.qdrant.data-layer.svc.cluster.local`). With a regular ClusterIP service, DNS only resolves to the virtual IP, and Qdrant's peer discovery cannot identify individual nodes.
</ImplementationNote>
...
<ExternalCite
  title="Qdrant Filtering Documentation"
  url="https://qdrant.tech/documentation/concepts/filtering/"
  author="Qdrant"
  date="2024"
/>

<ImplementationNote title="Payload Indexes for Multi-Tenant Filtering">
Payload indexes on `owner_id` and `document_id` are crucial for efficient filtering. Without them, every search would scan all vectors in the collection, which becomes catastrophic at scale. In our multi-tenant setup, the `owner_id` filter is applied on every single query, so indexing it was non-negotiable. I learned this the hard way when our staging environment (with ~100K vectors) had acceptable search times without the index, but production (with 500K+ vectors) was timing out. Adding the payload index brought query times from 2-3 seconds back down to under 50ms.
</ImplementationNote>
...
<ImplementationNote title="gRPC vs REST Performance">
The gRPC port (6334) gave us 3x better throughput than the REST API (6333) for batch operations like upserting hundreds of vectors during document ingestion. All production code uses gRPC exclusively, with REST only used for debugging via port-forward when I need to quickly inspect a collection or run a manual query from my terminal. The gRPC client also handles connection pooling and multiplexing more efficiently, which matters when multiple application pods are hitting the same Qdrant cluster concurrently.
</ImplementationNote>
...
| Protocol | gRPC (6334) for production, REST (6333) for debugging |

Combined with proper collection management and filtering, Qdrant enables efficient multi-tenant semantic search at scale.

### Next Steps

- Explore [Hybrid Search: Combining Semantic and Keyword Search](/hybrid-search-semantic-keyword) to pair vector search with BM25 keyword matching.
- Read about [Improving RAG Query Quality](/improving-rag-query-quality-and-relevance) for query rewriting and HyDE techniques.
- Consider adding snapshot-based backups and disaster recovery for your Qdrant collections.
