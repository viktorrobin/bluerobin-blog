...
tags: ["async", "background-jobs", "dotnet"]
readTime: "15 min"
toc: true
---
...
<ExternalCite title="Background tasks with hosted services in ASP.NET Core" url="https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services" author="Microsoft" date="2024-11-15" />
...
<ImplementationNote>
The graceful shutdown problem cost me several production incidents. When Kubernetes sends SIGTERM, the .NET host triggers the `CancellationToken`, but by default you only get 30 seconds before a hard kill. Our document processing jobs could take up to two minutes for large PDFs. I had to configure `HostOptions.ShutdownTimeout` to 120 seconds and add a `finally` block in the handler that marks in-flight jobs as "interrupted" in the database, so the next instance picks them up. Without that explicit state tracking, we had documents stuck in a "processing" state with no way to recover them automatically.
</ImplementationNote>
...
<ExternalCite title="System.Threading.Channels" url="https://learn.microsoft.com/en-us/dotnet/core/extensions/channels" author="Microsoft" date="2024-08-20" />
...
<ExternalCite title="PeriodicTimer Class" url="https://learn.microsoft.com/en-us/dotnet/api/system.threading.periodictimer" author="Microsoft" date="2024-09-10" />
...
<ExternalCite title="Polly v8 - Resilience and transient-fault-handling library" url="https://github.com/App-vNext/Polly" author="Polly Project" date="2024-10-05" />
...
<ImplementationNote>
Tuning the retry policy for background jobs was trickier than I expected. Our initial configuration had `MaxRetryAttempts = 5` with a base delay of 1 second, which meant a failing job could block a worker for over 30 seconds with exponential backoff. During a brief outage of our OCR service, all three workers ended up stuck in retry loops and the channel queue backed up to capacity, causing the API to block on enqueue. I reduced retries to 3, added the 5-minute timeout as a hard ceiling, and -- most critically -- added a dead-letter mechanism: after exhausting retries, the job gets persisted to a `failed_jobs` table with the exception details so we can replay it later.
</ImplementationNote>
...
<ExternalCite title="Scrutor - Assembly scanning and decoration for Microsoft.Extensions.DependencyInjection" url="https://github.com/khellang/Scrutor" author="Kristian Hellang" date="2024-07-22" />
...
<ExternalCite title="Microsoft: Background tasks with hosted services" url="https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services" author="Microsoft" date="2024" />

<ExternalCite title="System.Threading.Channels deep dive" url="https://learn.microsoft.com/en-us/dotnet/core/extensions/channels" author="Microsoft" date="2024" />

<ExternalCite title="Polly resilience and transient-fault-handling" url="https://github.com/App-vNext/Polly" author="GitHub Community" date="2024" />

<ExternalCite title="Scrutor for DI decoration patterns" url="https://github.com/khellang/Scrutor" author="GitHub Community" date="2024" />
<ExternalCite title="Background tasks with hosted services in ASP.NET Core" url="https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services" author="Microsoft" date="2024-11-15" />
