...
BlueRobin has since collapsed to a single production environment (ADR-034), so the staging overlays shown here no longer exist in our repo, and the `signoz-otel-collector` endpoints in the examples are now the Grafana LGTM stack's OTEL Collector. The overlay patterns themselves are unchanged and still what we use for prod.
</Callout>
...
<ExternalCite title="Kustomize - Kubernetes native configuration management" url="https://kustomize.io/" author="Kubernetes SIG CLI" date="2024-07-01" />
...
<ExternalCite title="Kustomize Glossary - Base" url="https://kubectl.docs.kubernetes.io/references/kustomize/glossary/#base" author="Kubernetes SIG CLI" date="2024-07-01" />
...
<ImplementationNote title="The namePrefix gotcha with HPA targets">
I spent a frustrating afternoon wondering why my HPA was not scaling pods in staging. The deployment was running fine, but `kubectl describe hpa` showed "unable to find target ref." The issue was that `namePrefix: staging-` was being applied to the Deployment name, making it `staging-myapp-api`, but I assumed the HPA's `scaleTargetRef.name` needed to match the original base name. Kustomize actually transforms the HPA reference automatically, but only if the HPA is listed in the same kustomization.yaml `resources`. Once I moved the HPA into the overlay's resource list, Kustomize correctly prefixed both the Deployment and the HPA target reference.
</ImplementationNote>
...
<ExternalCite title="Kustomize - Patches (Strategic Merge)" url="https://kubectl.docs.kubernetes.io/references/kustomize/kustomization/patches/" author="Kubernetes SIG CLI" date="2024-07-01" />
...
<ImplementationNote title="ConfigMap hash suffixes and rollout triggers">
I was puzzled when my configuration changes were not triggering pod rollouts. I would update a literal in `configMapGenerator`, Flux would reconcile, but the pods kept running with the old config. The reason is that Kustomize appends a hash suffix to ConfigMap names (e.g., `myapp-api-config-8k2g5m`), and the Deployment's `envFrom` reference is updated to match. However, I had `disableNameSuffixHash: true` on my staging ConfigMap for debugging convenience, which meant the ConfigMap name never changed, so Kubernetes saw no reason to restart pods. I removed that flag for staging and production, and now every config change automatically triggers a rolling update.
</ImplementationNote>

### Flux Integration

<ExternalCite title="Flux - Kustomize Controller" url="https://fluxcd.io/flux/components/kustomize/" author="Flux Project" date="2024-10-01" />
