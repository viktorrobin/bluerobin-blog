...
<ExternalCite
  title="Semantic Kernel: RAG Patterns and Best Practices"
  url="https://learn.microsoft.com/en-us/semantic-kernel/concepts/vector-store-connectors/rag"
  author="Microsoft"
  date="2024-06-20"
/>
...
<ExternalCite
  title="Qdrant Search API: Similarity Search with Filtering"
...
<ImplementationNote>
The system prompt went through 15+ iterations before I settled on the version above. The single most impactful change was adding "If the context doesn't contain relevant information, say so clearly" -- before that line, the model would confidently fabricate answers when the retrieved chunks were only tangentially related to the question. Temperature tuning was equally important: 0.0 produced robotic, sometimes repetitive answers that would get stuck in loops; 0.3 and above started introducing subtle hallucinations (inventing clause numbers, paraphrasing quotes inaccurately). I landed on 0.1 as the sweet spot -- just enough randomness to produce natural-sounding prose while keeping the model tightly grounded in the provided context.
</ImplementationNote>
...
<ImplementationNote>
The "Lost in the Middle" problem hit us hard in early testing. When I fed 10+ chunks to the LLM, the model would reliably use information from the first 2-3 sources and the last 1-2 sources, but almost entirely ignored chunks in positions 4-8. Users noticed because the cited sources never came from the middle of the context window. My fix was a two-part reranking strategy: first, I reordered chunks by relevance score, then I placed the highest-scoring chunk at position 1 and the second-highest at the last position, distributing the rest in between. This "sandwich" arrangement noticeably improved answer completeness. I also capped context at 5 chunks maximum for our 8B parameter model -- beyond that, quality degraded regardless of ordering.
</ImplementationNote>
...
<ExternalCite
  title="Ollama API Documentation: Generate"
...
- **[Building a Semantic Embedding Pipeline with Ollama and Qdrant](/embedding-pipeline-ollama-qdrant)** -- The companion article covering the embedding and indexing side of the pipeline in detail.
