...
category: "ai"
difficulty: "advanced"
tags: ["ai-agents", "dotnet", "kubernetes", "semantic-kernel", "tool-calling"]
readTime: "22 min"
ogImage: "/og/semantic-kernel-agents-orchestration.png"
canonicalURL: "https://blog.bluerobin.io/semantic-kernel-agents-orchestration"
toc: true
series:
  name: "AI & ML Pipeline"
  part: 6
deprecated: true
deprecatedReason: "Semantic Kernel orchestration was superseded by LangGraph-based .NET agent routing."
---
...
<ExternalCite title="ReAct: Synergizing Reasoning and Acting in Language Models" url="https://arxiv.org/abs/2210.03629" author="Yao, S. et al." date="2022-10-06" />
...
1. **Search Agent**: Semantic and keyword search over document collections
2. **Analysis Agent**: Document comparison and entity extraction
3. **Summary Agent**: Report generation and synthesis
4. **Agent Orchestrator**: Coordinates agents for complex multi-step tasks
...
<ExternalCite title="Semantic Kernel Overview" url="https://learn.microsoft.com/en-us/semantic-kernel/overview/" author="Microsoft" date="2024-11-15" />

<ExternalCite title="Semantic Kernel GitHub Repository" url="https://github.com/microsoft/semantic-kernel" author="Microsoft" date="2024-12-01" />
...
<ExternalCite title="AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation" url="https://arxiv.org/abs/2308.08155" author="Wu, Q. et al. (Microsoft Research)" date="2023-08-16" />
...
1. **User Query**: Natural language request enters the system
2. **Planner**: LLM reasons about which agents/tools to invoke
3. **Agents**: Execute their specialized plugins (search, analyze, summarize)
4. **Memory**: Maintains conversation context and semantic recall
...
2. Orchestrator uses LLM reasoning to determine which agents/tools to invoke
3. Agents execute their tools (search, read, analyze, etc.)
4. Results are combined and synthesized into a coherent response
...
<ExternalCite title="Semantic Kernel Plugins Documentation" url="https://learn.microsoft.com/en-us/semantic-kernel/concepts/plugins/" author="Microsoft" date="2024-10-20" />
...
<ImplementationNote>
Each plugin function has a clear `[Description]` that helps the LLM decide when to use it. Be specific about when each tool should be used. Early on, I found the planner's automatic function calling was unpredictable in production -- the LLM would call the wrong plugin or chain them in nonsensical orders. After two weeks of fighting non-deterministic behavior, I switched to explicit orchestration with deterministic routing, using SK plugins as building blocks rather than letting the planner decide everything. The key insight was using `ToolCallBehavior.AutoInvokeKernelFunctions` only for the final synthesis step, while routing search and analysis through a hardcoded pipeline. This gave us 95% of the flexibility with 100% of the predictability.
</ImplementationNote>
...
<ImplementationNote>
Plugin execution timeout was a lesson I learned the hard way. One of our external API plugins -- the one that called a third-party entity extraction service -- hung for over 2 minutes during a demo, blocking the entire agent pipeline. The user saw a spinning cursor and eventually got a gateway timeout. After that incident, I added a 10-second timeout per plugin with graceful degradation: if the entity extraction plugin times out, the agent continues with whatever data it already has and notes the limitation in its response. We wrapped every plugin call in a `CancellationTokenSource` with a configured timeout, and the difference in reliability was immediate.
</ImplementationNote>
...
<ExternalCite title="Function Calling - OpenAI API" url="https://platform.openai.com/docs/guides/function-calling" author="OpenAI" date="2024-06-01" />

<ImplementationNote>
Memory management with SK's chat history was a silent scaling problem. In development, conversations were short and everything worked. In production, power users would have 50+ message conversations, and the chat history grew unbounded -- eventually exceeding the model's context window. The LLM started "forgetting" early instructions and plugin descriptions. I implemented a sliding window of the last 10 messages plus the system prompt, with a compressed summary of earlier messages generated by a fast local model. This kept token usage predictable while preserving conversation coherence. The summary generation adds about 200ms per turn, but it is a worthwhile trade-off.
</ImplementationNote>
...
- [LangGraph State Collisions: Lessons Learned](./langgraph-state-collision-lessons) -- state management pitfalls in another agent framework
- [AI Strategy: Moving from Local Llama to OpenAI](./ai-strategy-moving-local-llama-to-openai) -- the hybrid architecture powering our LLM backend
- [Optimizing System Latency](./latency-revolution-optimizing-60s-to-3s) -- reducing end-to-end response times

### Further Reading

<ExternalCite title="Semantic Kernel Documentation" url="https://learn.microsoft.com/en-us/semantic-kernel/" author="Microsoft" date="2024-11-15" />
