...
<ExternalCite title="FalkorDB Documentation" url="https://docs.falkordb.com/" author="FalkorDB" date="2024-09-10" />

Graph databases represent data as nodes and edges rather than rows and columns. This makes traversal queries -- "find all entities connected to X within 3 hops" -- orders of magnitude faster than equivalent SQL joins, because the graph engine follows pointers rather than scanning index tables.

<ExternalCite title="Graph Databases: New Opportunities for Connected Data (2nd ed.)" url="https://www.oreilly.com/library/view/graph-databases-2nd/9781491930885/" author="Ian Robinson, Jim Webber, and Emil Eifrem" date="2015-06-10" />
...
<ExternalCite title="StackExchange.Redis Documentation" url="https://stackexchange.github.io/StackExchange.Redis/" author="Stack Exchange" date="2024-10-05" />
...
<ExternalCite title="OpenCypher: An Open Standard for the Cypher Query Language" url="https://opencypher.org/" author="openCypher Project" date="2024-05-15" />
...
<ImplementationNote>
FalkorDB returns query results as nested Redis arrays, and parsing them into strongly-typed C# objects was by far the most fragile part of the integration. The response format is undocumented at the level of detail you need for a generic parser -- you get an array of header names, then an array of row arrays, but the value types are inconsistent (strings sometimes come back as Redis bulk strings, sometimes as simple strings). This code broke twice during minor FalkorDB version upgrades when the serialization format changed slightly. I ended up pinning the FalkorDB Docker image to a specific version (`v4.2.0`) and adding integration tests that verify the parse behavior against known query results. If FalkorDB ever publishes an official .NET client, I would switch to it immediately.
</ImplementationNote>
...
<ImplementationNote>
The MERGE-based upsert pattern (`MERGE (e:Person {name: $name, ownerId: $ownerId})`) was critical for idempotent entity creation -- the same entity mentioned across multiple documents should be a single node, not duplicates. But MERGE performance degraded badly once the graph grew past a few thousand nodes, because FalkorDB was scanning all nodes of the matching label to check for existence. The fix was creating composite indexes on `(name, ownerId)` for every entity label (Person, Organization, Location, etc.). Without the index, a MERGE on a graph with 10K Person nodes took 200ms; with the index, it dropped to under 5ms. Always create indexes before you need them -- retrofitting them on a production graph is painful.
</ImplementationNote>
...
<ImplementationNote>
The orphaned entity cleanup query (`MATCH (e) WHERE NOT (e)--() AND NOT e:Document DELETE e`) in `RemoveDocumentEntitiesAsync` turned out to be surprisingly expensive. On a graph with 50K+ nodes, it would scan every node and check its degree, taking 5-10 seconds and blocking other queries. I moved this out of the synchronous document-deletion path and into a nightly scheduled job instead. The trade-off is that orphaned nodes exist for up to 24 hours, but since they have no relationships, they do not affect query results -- they just consume a small amount of memory. For most knowledge graph use cases, eventual cleanup is perfectly acceptable.
</ImplementationNote>
...
<ExternalCite title="A Survey on Knowledge Graphs: Representation, Acquisition, and Applications" url="https://arxiv.org/abs/2002.00388" author="Shaoxiong Ji et al." date="2022-04-01" />
...
- [NER with spaCy for Document Analysis](/articles/spacy-ner-document-analysis) -- the entity extraction pipeline that feeds this graph
- [Ensemble NER: spaCy + LLM Voting](/articles/ensemble-ner-spacy-llm-voting) -- improve entity quality with multi-provider ensembling before graph ingestion

### Further Reading

<ExternalCite title="FalkorDB Documentation" url="https://docs.falkordb.com/" author="FalkorDB" date="2024-09-10" />

<ExternalCite title="Neo4j Cypher Manual" url="https://neo4j.com/docs/cypher-manual/current/" author="Neo4j" date="2024-11-01" />

<ExternalCite title="OpenCypher Specification" url="https://opencypher.org/" author="openCypher Project" date="2024-05-15" />
