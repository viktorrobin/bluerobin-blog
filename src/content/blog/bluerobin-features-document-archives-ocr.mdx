...
The storage layer described below has changed: documents now live in Cloudflare R2 under a per-user key prefix, encrypted client-side with per-user envelope encryption before they leave the cluster — MinIO and KES are retired (ADR-006). The rest of the pipeline in this article is still current.
</Callout>
...
BlueRobin computes a SHA-256 fingerprint per document and stores it in both PostgreSQL and MinIO. Uploading the same file twice is silently rejected before any processing begins. Identical content under different filenames is caught the same way.
</Callout>
...
- **[MinIO Multi-Tenant Storage](/articles/minio-kubernetes-multi-tenant)** -- the per-user bucket architecture that stores every uploaded file.
...
<ExternalCite
  title="MinIO Server-Side Encryption with KMS"
