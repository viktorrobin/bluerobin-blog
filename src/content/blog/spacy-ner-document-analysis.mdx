...
When I first uploaded a scanned legal contract into the document pipeline and watched the NER service run, I was genuinely amazed. Within seconds, the system had automatically identified every party name, every effective date, and every monetary value buried in the dense legalese -- turning a completely unstructured PDF into structured, searchable data. That moment sold me on the power of named entity recognition as a foundational building block for document intelligence, and it shaped how I designed the rest of the extraction pipeline.
...
<ExternalCite title="spaCy Transformer Models (spacy-transformers)" url="https://spacy.io/usage/embeddings-transformers" author="Explosion AI" date="2024-06-01" />
...
<ImplementationNote>
I learned the hard way that the `en_core_web_trf` model takes 8-12 seconds to load at startup. During my first deployment, the Kubernetes readiness probe was set to the default 10 seconds, which meant the pod kept getting killed before it could finish loading the model -- an endless restart loop. I had to bump `initialDelaySeconds` to 30 and `livenessProbe` to 60 before things stabilized. If you are running transformer models in containers, always account for the cold-start penalty in your health check configuration.
</ImplementationNote>
...
The entity types we extract (PERSON, ORG, GPE, DATE, MONEY, etc.) follow the OntoNotes 5 annotation scheme, which defines 18 entity categories designed for broad-coverage NER.
...
<ImplementationNote>
One thing that tripped me up: spaCy does not provide native confidence scores for extracted entities. Unlike Hugging Face token classification models that output per-token probabilities, spaCy's pipeline simply returns entities without any score. The heuristic I settled on -- `min(0.9, 0.6 + (len(ent.text) / 50))` -- is admittedly a pragmatic workaround rather than a principled solution. It biases toward longer entity text being more confident, which works reasonably well in practice (single-word entities like "Apple" are genuinely more ambiguous than "Apple Inc."), but I would not rely on these scores for anything critical without calibrating against labeled data.
</ImplementationNote>
...
For production workloads that require true confidence scores, Hugging Face's token classification models expose per-token probabilities that can be aggregated to entity-level scores.

<ExternalCite title="Hugging Face NER Models - Token Classification" url="https://huggingface.co/models?pipeline_tag=token-classification" author="Hugging Face" date="2024-12-01" />
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: spacy-ner
          image: registry.example.com/myapp/spacy-ner:latest
          ports:
            - containerPort: 8080
          resources:
            requests:
...
<ImplementationNote>
Processing large documents (100K+ characters) caused out-of-memory kills in the spaCy container. The transformer model's attention mechanism scales quadratically with input length, and a single 100K-character document could spike memory usage past the 4GB limit. The fix was to chunk text into 10K-character segments before sending to the NER endpoint, then adjust the character offsets in the response to map back to the original document positions. This added complexity to the client but eliminated the OOM crashes entirely. The 100K hard limit in the endpoint is a safety net, but the real solution lives in the chunking logic.
</ImplementationNote>
...
<ExternalCite title="Polly: Resilience and Transient-Fault Handling Library" url="https://github.com/App-vNext/Polly" author="App-vNext" date="2024-08-15" />
...
public sealed class EntityExtractionWorker : BackgroundService
{
        ...
        CancellationToken ct)
    {
        var document = await _documents.GetByIdAsync(
            CustomId.From(evt.DocumentId), ct);
...
Building this NER pipeline changed how I think about document processing. What started as a "nice to have" feature quickly became the backbone of the entire knowledge graph -- every downstream feature (graph queries, faceted search, document clustering) depends on reliable entity extraction. The most valuable lesson was that accuracy at this stage compounds: a 5% improvement in NER precision means thousands fewer spurious nodes in the graph over time.
...
- [Ensemble NER: Combining spaCy + LLM with Voting](/articles/ensemble-ner-spacy-llm-voting) -- improve recall by adding an LLM-based NER provider and using confidence-aware voting
- [FalkorDB Knowledge Graph](/articles/falkordb-graph-database-knowledge-graph) -- store and query entity relationships using Cypher

### Further Reading

<ExternalCite title="spaCy NER Documentation" url="https://spacy.io/usage/linguistic-features#named-entities" author="Explosion AI" date="2024-06-01" />

<ExternalCite title="spaCy Transformer Models (spacy-transformers)" url="https://spacy.io/usage/embeddings-transformers" author="Explosion AI" date="2024-06-01" />

<ExternalCite title="Explosion AI Blog: NER Best Practices" url="https://explosion.ai/blog" author="Explosion AI" date="2024-03-15" />
...
<ExternalCite title="Hugging Face NER Models" url="https://huggingface.co/models?pipeline_tag=token-classification" author="Hugging Face" date="2024-12-01" />
