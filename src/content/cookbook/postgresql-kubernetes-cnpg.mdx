...
<Callout type="info">
The multi-database (`myapp_staging` / `myapp_prod`) setup below is a generic teaching example—hosting several environment databases in one CNPG cluster is a valid pattern. BlueRobin itself now runs **prod-only in-cluster with development on a laptop** (ADR-034), so its in-cluster CNPG cluster serves a single production database.
</Callout>
...
- **[PostgreSQL Security Design](/blog/postgresql-security-design-high-assurance)** -- harden your CNPG cluster with row-level security and audit logging.
- **[External Secrets with Infisical](/blog/external-secrets-infisical-kubernetes)** -- manage database credentials through Infisical instead of static secrets.
- **[Infrastructure Performance: SSD vs NAS](/blog/infrastructure-performance-ssd-vs-nas)** -- choose the right storage backend for your PostgreSQL data.

### Further Reading

<ExternalCite
  title="CloudNative-PG Documentation"
  url="https://cloudnative-pg.io/documentation/current/"
  author="CloudNative-PG Community"
  date="2024"
/>

<ExternalCite
  title="PostgreSQL High Availability with Kubernetes"
  url="https://www.enterprisedb.com/blog/postgresql-kubernetes-high-availability"
  author="EDB"
  date="2024"
/>
