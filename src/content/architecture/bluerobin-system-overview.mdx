...
<ImplementationNote>
Events use environment-prefixed subjects (e.g., `dev.archives.documents.ocr.requested`, `staging.archives.documents.ocr.requested`) to isolate environments on a shared NATS cluster.
</ImplementationNote>
...
PostgreSQL stores structured data with per-environment databases. I run PostgreSQL via the CloudNativePG operator, which handles automatic failover, backups, and replication—it's one of the best Kubernetes operators I've worked with.
